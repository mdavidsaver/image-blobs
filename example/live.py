...
class App:
    ...
    def on_image(self, img):
        ...
        try:
            ...
            for F in features[:1]: # split out (first) largest feature
                self.X.post(F['X'])
                self.Y.post(F['Y'])
                self.W.post(F['W'])
                self.H.post(F['H'])

            self.features.post(features) # publish table of all features

            img = show_features(img, features) # render output image
